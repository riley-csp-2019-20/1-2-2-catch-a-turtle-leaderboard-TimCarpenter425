...
def load_leaderboard(file_name, leader_names, leader_scores):
  ...
  leaderboard_file = open("a122_leaderboard.txt", "r")  # need to create the file ahead of time in same folder
...
# update_leaderboard("a122_leaderboard.txt", ["a", "b", "c"], [7, 4, 3], "d", 6)
...
def draw_leaderboard(leader_names, leader_scores, high_scorer, turtle_object, player_score):
  ...
  if (high_scorer):
    ...
    turtle_object.write("Sorry, you didn't make the leaderboard.", font=font_setup)
  ...
  # TODO 10: Display a platinum/gold/silver/bronze message if player earned a gold/silver/or bronze medal; display nothing if no medal
  
  if(player_score < bronze_score):
    turtle_object.write("You did not earn any medal :(", font=font_setup)
  ...
  if(player_score >= gold_score and player_score < platinum_score):
    turtle_object.write("You earned a gold medal!", font=font_setup)
  if(player_score >= platinum_score):
    turtle_object.write("You earned a platinum medal!", font=font_setup)